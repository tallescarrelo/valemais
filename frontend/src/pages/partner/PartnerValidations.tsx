...
import styles from './PartnerValidations.module.scss';
...
export default function PartnerValidations() {
  ...
  async function loadValidations() {
    setLoading(true);
    try {
      const res = await partnerService.getValidations(page, LIMIT);
      const data = res.data;
      ...
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            Historico de descontos validados no seu estabelecimento
          </p>
        </div>
        {total > 0 && (
        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              description="Quando clientes VALE+ validarem descontos no seu estabelecimento, elas aparecerao aqui."
            />
          </CardBody>
        </Card>
      ) : (
                      ...
                      <tr key={v.id || index}>
                        ...
                        <td>
                          <div className={styles.clientCell}>
                            <User size={16} className={styles.clientIcon} />
                            <span>{v.clientName || v.client?.name || 'Cliente'}</span>
                          </div>
                        </td>
                        <td>
                          <span className={styles.cardCodeCell}>
                            {v.cardCode || v.card?.code || '--'}
                          </span>
                        </td>
                        <td>
                  ...
                  <CardBody>
                    <div className={styles.validationCardHeader}>
                      ...
                      <div className={styles.validationInfo}>
                        ...
                        <span className={styles.validationCardCode}>
                          {v.cardCode || v.card?.code || '--'}
                        </span>
                      </div>
